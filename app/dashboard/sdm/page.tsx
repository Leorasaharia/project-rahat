...
export default function SDMDashboard() {
  ...
  useEffect(() => {
    ...
    if (!storedUsername || storedRole !== "sdm") {
      router.push("/login")
      return
    }
    ...
    if (storedApplicants) {
      ...
      // Filter only applications sent to SDM
      const sdmApplicants = allApplicants.filter((app: Applicant) => app.status === "sent-to-sdm" || app.sdmStatus)
      setApplicants(sdmApplicants)
    }
  ...
  const handleApprove = (applicantId: string) => {
    const updatedApplicants = applicants.map((app) =>
      app.id === applicantId ? { ...app, sdmStatus: "approved" as const, sdmComments: comments } : app,
    )
  ...
  const handleReject = (applicantId: string) => {
    const updatedApplicants = applicants.map((app) =>
      app.id === applicantId ? { ...app, sdmStatus: "rejected" as const, sdmComments: comments } : app,
    )
    setApplicants(updatedApplicants)
  ...
  const handleForwardToNext = (applicantId: string) => {
    const updatedApplicants = applicants.map((app) =>
      app.id === applicantId ? { ...app, status: "forwarded-to-rahat" } : app,
    )
    setApplicants(updatedApplicants)
    ...
    setAlertMessage("Application forwarded to Rahat Operator!")
    setShowAlert(true)
    setTimeout(() => setShowAlert(false), 5000)
  }

  const getStatusBadge = (applicant: Applicant) => {
    if (applicant.sdmStatus === "approved") {
      return (
    ...
    } else if (applicant.sdmStatus === "rejected") {
      return (
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div className="text-white">
                <h1 className="text-xl font-bold">SDM PERFORMANCE DASHBOARD</h1>
                <p className="text-sm opacity-90">एसडीएम प्रदर्शन डैशबोर्ड | GOVERNMENT OF CHHATTISGARH</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <LanguageToggle />
              <div className="text-white text-right">
                ...
                <p className="text-xs opacity-80">Sub Divisional Magistrate</p>
              </div>
          ...
          <CardContent>
            {applicants.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p>No applications pending for SDM review.</p>
              </div>
            ) : (
                  ...
                  <Card key={applicant.id} className="border-l-4 border-l-blue-500">
                    <CardContent className="p-4">
                      ...
                      <div className="grid md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <ul className="text-sm text-gray-600">
                            <li>Patwari: {applicant.verifications.patwari ? "✓ Verified" : "✗ Not Verified"}</li>
                            <li>
                              Thana In-charge: {applicant.verifications.thanaIncharge ? "✓ Verified" : "✗ Not Verified"}
                            </li>
                          </ul>
                          <p className="mt-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {applicant.sdmStatus === "approved" && applicant.status !== "forwarded-to-rahat" && (
                          <Button
                            onClick={() => handleForwardToNext(applicant.id)}
                            className="bg-blue-500 hover:bg-blue-600 text-white"
                          >
                            <Send className="w-4 h-4 mr-2" />
                            Forward to Rahat Operator
                          </Button>
                        )}
                      </div>
                    </CardContent>
