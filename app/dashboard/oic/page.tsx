...
export default function OICDashboard() {
  ...
  useEffect(() => {
    ...
    if (!storedUsername || storedRole !== "oic") {
      router.push("/login")
      return
    }
    ...
    if (storedApplicants) {
      ...
      // Filter applications forwarded to OIC
      const oicApplicants = allApplicants.filter((app: Applicant) => app.status === "forwarded-to-oic" || app.oicStatus)
      setApplicants(oicApplicants)
    }
  ...
  const handleApprove = (applicantId: string) => {
    const updatedApplicants = applicants.map((app) =>
      app.id === applicantId ? { ...app, oicStatus: "approved" as const, oicComments: comments } : app,
    )
    ...
    setAlertMessage("Application approved by OIC!")
    setShowAlert(true)
  ...
  const handleReject = (applicantId: string) => {
    const updatedApplicants = applicants.map((app) =>
      app.id === applicantId ? { ...app, oicStatus: "rejected" as const, oicComments: comments } : app,
    )
    setApplicants(updatedApplicants)
  ...
  const handleForwardToNext = (applicantId: string) => {
    const updatedApplicants = applicants.map((app) =>
      app.id === applicantId ? { ...app, status: "forwarded-to-adg" } : app,
    )
    setApplicants(updatedApplicants)
    ...
    setAlertMessage("Application forwarded to ADG!")
    setShowAlert(true)
    setTimeout(() => setShowAlert(false), 5000)
  }

  const getStatusBadge = (applicant: Applicant) => {
    if (applicant.oicStatus === "approved") {
      return (
    ...
    } else if (applicant.oicStatus === "rejected") {
      return (
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div className="text-white">
                <h1 className="text-xl font-bold">OIC PERFORMANCE DASHBOARD</h1>
                <p className="text-sm opacity-90">ओआईसी प्रदर्शन डैशबोर्ड | GOVERNMENT OF CHHATTISGARH</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <LanguageToggle />
              <div className="text-white text-right">
                ...
                <p className="text-xs opacity-80">Officer In Charge</p>
              </div>
              {userProfile && (
          ...
          <CardContent>
            {applicants.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p>No applications pending for OIC review.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {applicants.map((applicant) => (
                  <Card key={applicant.id} className="border-l-4 border-l-purple-500">
                    <CardContent className="p-4">
                      ...
                      <div className="flex space-x-2">
                        ...
                        {applicant.oicStatus === "approved" && applicant.status !== "forwarded-to-adg" && (
                          <Button
